...
│                                                            │
│  Encoder: backbone TBD bằng benchmark thật (xem 4.0a) —  │
│           fine-tune contrastive (SetFit-style) trên       │
│           agent_enriched (group_a + group_b)               │
...
### 4.0a Chọn encoder: bge-small vs ModernBERT — chưa quyết, cần đo

Hai backbone hợp lý cho bước fine-tune contrastive, đánh đổi khác nhau, **chưa có số liệu trên đúng dataset này** để chọn dứt điểm:

| | `bge-small-en-v1.5` (đang dùng ở Stage 3 hiện tại) | `ModernBERT-base` |
|---|---|---|
| Params / dim | 33M / 384-dim | 149M / 768-dim |
| Pretrain gốc | Contrastive sentence-similarity (MS MARCO, NLI...) — đã tối ưu cho similarity trước khi fine-tune | MLM hiện đại (rotary embeddings, alternating local/global attention, unpadding) — không pretrain riêng cho similarity, nhưng benchmark SetFit công khai cho thấy làm base tốt hơn các backbone cũ khi fine-tune | 
| Evidence định lượng | Frozen, dùng nguyên trong code hiện tại — chưa từng fine-tune | *SetFit ModernBERT*: 92.7% accuracy, 8 example/class trên IMDB — **không phải trên data ERC-8004** |
| Latency CPU (ước tính) | Thấp hơn rõ do nhỏ hơn 4.5x | Cao hơn, nhưng ModernBERT thiết kế riêng để hiệu quả hơn kích thước (unpadding, flash-attention-style) — chưa đo thật trên input ngắn của bài toán này |
| Context length cần | Input ngắn (tag + agent text) — không cần | 8192 token — overkill, không tận dụng được |
| Rủi ro vận hành | Thấp — không đổi infra | Throughput hệ thống đang ở 330 msg/s/core có thể giảm — cần đo, không giả định |

**Quyết định**: không chọn trước theo cảm tính. Bước 2 trong kế hoạch triển khai (mục 5) phải benchmark **cả hai backbone** trên cùng `agent_enriched` train set + cùng gold validation set, đo cả accuracy (risk-coverage curve) và latency thật trên CPU, rồi chọn theo kết quả đo được — không theo benchmark public (IMDB) vì domain khác hoàn toàn.
...
| 2a | Viết `benchmarks/train_unified_classifier.py` với `--backbone` tham số hoá (`bge-small` \| `modernbert-base`): load `group_a.parquet` + `group_b.parquet`, build fused text, fine-tune encoder + train 3-class head | mới | Cần `data/splits/agent_enriched/*.parquet` (đã có sẵn) |
| 2b | Chạy bước 2a với **cả hai backbone**, đo accuracy (risk-coverage curve, mục 4.2) và latency CPU thật (ms/record) trên cùng gold validation set — chọn backbone theo số đo, không theo benchmark public | — | Không |
| 3 | Viết script tính risk-coverage curve trên validation split (dùng cho cả bước 2b lẫn calibrate τ cuối), xuất τ tối ưu theo coverage mục tiêu | `benchmarks/calibrate_reject_threshold.py` (mới) | Không |
| 4 | Viết `shared/unified_classifier.py`: load model đã chọn ở 2b, expose `classify(tag1, tag2, scale, agent_domain_text) -> (category, prob, confident: bool)` | mới | Artifact từ bước 2b |
...
| 6 | Re-run `benchmarks/eval_pure_others.py`-style evaluation trên CHÍNH module mới để so sánh Macro F1 / coverage / LLM-call-rate / latency với Run 13 | sửa hoặc bản mới của `eval_pure_others.py` | Không |
| 7 | A/B hoặc shadow-traffic so sánh trước khi chuyển production traffic | ops, ngoài repo | Không |

**Lưu ý quan trọng**: bước 2b và bước 6 bắt buộc — phải đo trên cùng gold set (`data/labelled/pure_others_to_label.csv`) trước khi (a) chọn backbone và (b) quyết định chuyển production, để có số liệu so sánh thật, không chỉ dựa trên benchmark public hoặc lý thuyết.
