#!/usr/bin/env python3
"""Run 11: Rule + bge-SVM (no scale) + FAISS + LLM (improved junk prompt).
...
Usage:
    ...
    .venv/bin/python3 -m benchmarks.pipeline_run11 \
...
def main() -> None:
    ...
    print(f"\n{'='*60}")
    print(f"  Run 11: Rule + bge-SVM (no scale) + FAISS + LLM (V8 junk fix)")
    ...
    audit_path = OUT_DIR / f"audit_run11_{ts}.csv"
    audit_df.to_csv(audit_path, index=False)
    print(f"\n  Audit saved to {audit_path}")

    out_path = OUT_DIR / f"pipeline_run11_{ts}.json"
    out_path.write_text(json.dumps({
        "name": "Run 11: Rule + bge-SVM (no scale) + FAISS + LLM (V8 junk fix)",
