...
2. **Research notebooks** under [`notebooks/`](notebooks/) — benchmark approaches compared on the same gold set (detailed in Chapter 6):

| Approach | Notebook |
|---|---|
| Classical ML (Naive Bayes) | `02_approach_a_classical.ipynb` |
| Frozen Embedding + Classical ML | `03_approach_b_embedding.ipynb` |
| Zero-shot LLM | `04_approach_c_llm.ipynb` |
| Fine-Tuned Unified Encoder (ModernBERT) | `05_approach_d_finetune.ipynb` |
| Production Cascade (BGE-SVM + LLM escalation) | `06_approach_e_cascade.ipynb` |
...
The LLM emits one of the first three (it never returns `others`); `others` is the rule-classifier's fallback bucket for rows that didn't match any rule.
...
3. `02_approach_a_classical.ipynb`, `03_...`, `04_...`, `05_...`, `06_...` — each writes results to `data/results/<approach>.parquet`
4. `07_evaluation.ipynb` — loads all results, prints comparison table, saves plots
